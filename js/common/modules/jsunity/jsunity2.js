...
var jsUnity = exports.jsUnity = (function () {
  ...
  var resultsStream = {
    ...
    fail: function (index, testName, message) {
      ...
      jsUnity.tap.write("  ---");
      jsUnity.tap.write("  stack: |\n" + message.replace(/^/mg, '      '));
      jsUnity.tap.write("  ...");
      jsUnity.log.info(fmt("[FAILED] ?: ?", testName, message));
    },

    skip: function(index, testName, reason) {
      jsUnity.tap.write(fmt("ok ? - ? # SKIP ?", index, testName, reason));
    },
  ...
  return {
    ...
    run: function (suites, matcher) {
      ...
      let jsUnitySuites = suites.map(suite => {
        suiteObject = suite();
        suiteObject.suiteName = suite.name;
        return jsUnity.compile(suiteObject);
      })

      let isMatchingTest = function(test) {
        return test.name.match(matcher);
      }

      let totalCount = jsUnitySuites.reduce((total, suite) => {
        return total + suite.tests.filter(isMatchingTest).length;
      }, 0);

      this.results.begin(totalCount);//, 'Total');
      results.total = totalCount;
      let testNumber = 0;
      jsUnitySuites.forEach(suite => {
        let matchingTests = suite.tests.filter(isMatchingTest);
        ...
        if (runSuite) {
          jsUnity.tap.write("# " + suite.suiteName);

          for (var j = 0; j < matchingTests.length; j++) {
            var test = matchingTests[j];
            if (test.fn.skip) {
              let skipResult = test.fn.skip();
              if (skipResult) {
                this.results.skip(++testNumber, test.name, skipResult);
                continue;
              }
            }
            ...
            try {
              ...
              this.results.pass(++testNumber, test.name);
              ...
              if (e.stack !== undefined) {
                this.results.fail(++testNumber, test.name, e + " - " + String(e.stack));
              }
              else {
                this.results.fail(++testNumber, test.name, e);
              }
            }
          }
        }
